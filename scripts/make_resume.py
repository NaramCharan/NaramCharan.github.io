...
# Stamp a content hash into the site so the download URL changes whenever the PDF
# does. GitHub Pages serves this file with `cache-control: max-age=600`, and phone
# browsers hold a downloaded PDF far longer than that — without a versioned URL a
# visitor who grabbed an older copy keeps getting it back from their own cache.
digest = hashlib.sha256(open(OUT, "rb").read()).hexdigest()[:8]
with open("lib/resumeVersion.ts", "w") as fh:
    fh.write(
        ...
        f'export const RESUME_VERSION = "{digest}";\n'
    )
print(f"stamped lib/resumeVersion.ts -> {digest}")
